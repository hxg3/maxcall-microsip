...
void Dialer::RebuildShortcuts(bool init)
{
	...
	if (shortcuts.GetCount()) {
		...
		if (mainDlg->shortcutsBottom) {
			...
			shortcutsRect.top = rectLast.bottom + mapRect.top;
		...
		for (int i = 0; i < shortcuts.GetCount(); i++) {
			...
			if (mainDlg->shortcutsBottom) {
				...
				if (shortcuts.GetCount() > 12) {
					int row = i % 2;
					button->Create(Translate(shortcut.label.GetBuffer()), style, CRect(shortcutsRect.right - 198 + row * 97, shortcutsRect.top, shortcutsRect.right - 4 - (1 - row) * 97, shortcutsRect.top + buttonHeight), this, IDC_SHORTCUT_RANGE + i);
					...
					button->Create(Translate(shortcut.label.GetBuffer()), style, CRect(shortcutsRect.right - 138, shortcutsRect.top, shortcutsRect.right - 4, shortcutsRect.top + buttonHeight), this, IDC_SHORTCUT_RANGE + i);
	...
	if (shortcuts.GetCount() == shortcutButtons.GetCount()) {
		for (int i = 0; i < shortcuts.GetCount(); i++) {
			...
			if (shortcut->presence) {
				...
				shortcutButtons.GetAt(pos)->SetIcon(
					mainDlg->imageListStatus->ExtractIcon(shortcut->image)
				);
				shortcutButtons.GetAt(pos)->RedrawWindow();
			}
		}
	}
}

void Dialer::PresenceReceived(CString* buddyNumber, int image, bool ringing)
{
	if (shortcuts.GetCount() == shortcutButtons.GetCount()) {
		bool blink = false;
		for (int i = 0; i < shortcuts.GetCount(); i++) {
			...
			if (shortcut->presence) {
				CString commands;
				CString numberFormated = FormatNumber(shortcut->number, &commands, true);
	...
	if (shortcuts.GetCount() == shortcutButtons.GetCount()) {
		for (int i = 0; i < shortcuts.GetCount(); i++) {
			...
			if (shortcut->ringing) {
				...
				POSITION pos = shortcutButtons.FindIndex(i);
				shortcutButtons.GetAt(pos)->SetIcon(
					mainDlg->imageListStatus->ExtractIcon(
						blinkState ? shortcut->image : MSIP_CONTACT_ICON_BLANK
					)
				);
				shortcutButtons.GetAt(pos)->RedrawWindow();
			}
...
{
	CButton *button = (CButton*)GetDlgItem(IDC_DIALER_FWD);
	accountSettings.FWD = button->GetCheck() == BST_CHECKED;
